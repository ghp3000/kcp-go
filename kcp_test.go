...
func testlink(t *testing.T, client *lossyconn.LossyConn, server *lossyconn.LossyConn, nodelay, interval, resend, nc int) {
	...
	sess, _ := NewConn2(server.LocalAddr(), nil, 0, 0, client, true, defaultKCPOptions)
	listener, _ := ServeConn(nil, 0, 0, server, true, defaultKCPOptions)
...
func BenchmarkFlush(b *testing.B) {
	kcp := NewKCP(1, defaultKCPOptions, func(buf []byte, size int) {})
