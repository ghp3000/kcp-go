...
func dialEcho(port int) (*UDPSession, error) {
	return dialEcho2(port, true)
}

func dialEcho2(port int, fullEncrypt bool) (*UDPSession, error) {
...
func listenEcho(port int) (net.Listener, error) {
	return listenEcho2(port, true)
}

func listenEcho2(port int, fullEncrypt bool) (net.Listener, error) {
	//block, _ := NewNoneBlockCrypt(pass)
	//block, _ := NewSimpleXORBlockCrypt(pass)
	//block, _ := NewTEABlockCrypt(pass[:16])
	//block, _ := NewAESBlockCrypt(pass)
...
func echoServer(port int) net.Listener {
	...
	go echoServerAcceptClient(l)

	return l
}

func echoServerNoFullEncryption(port int) net.Listener {
	l, err := listenEcho2(port, false)
	if err != nil {
		panic(err)
	}
...
func echoServerAcceptClient(l net.Listener) {
...
func testSendRecv(cli *UDPSession, t *testing.T) {
...
func TestNoFullEncryptionSendRecv(t *testing.T) {
	port := int(atomic.AddUint32(&baseport, 1))
	l := echoServerNoFullEncryption(port)
	defer l.Close()

	cli, err := dialEcho2(port, false)
	if err != nil {
		panic(err)
	}
	testSendRecv(cli, t)
